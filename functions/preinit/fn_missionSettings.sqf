if (isServer) then
{
	...
	escalationAmount = {
		round (playersNumber west / 10 * 3);
	};

	minWave = {
		round (playersNumber west / 10 * 2);
	};

	maxWave = {
		round (playersNumber west / 10 * 4);
	};

	maxVehicles = {
		ceil ((count spawnQueue) / 2);
	};

	activeGroups = {
		allGroups select { side _x == east && count (units _x) >= 4 && isNull objectParent (leader _x) };
	};
